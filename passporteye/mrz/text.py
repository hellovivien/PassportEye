...
class MRZ(object):
    ...
    def _guess_type(mrz_lines):
        ...
        try:
            if len(mrz_lines) == 1:
                ...
                return 'FR1' if mrz_lines[0][:4].upper() == "IDFR" else 'MRVB' if mrz_lines[0][0].upper() == 'V' else 'TD2'
    ...
    def _parse(self, mrz_lines):
        ...
        # try:
        if self.mrz_type == 'FR1':
            self.valid = self._parse_fr1(*mrz_lines)
        elif self.mrz_type == 'DLFR':
            self.valid = self._parse_dlfr(*mrz_lines)            
        elif self.mrz_type == 'TD1':
    ...
    def to_dict(self):
        ...
        if self.mrz_type is not None:
            ...
            result['valid_expiration_date'] = self.valid_check_digits[1]
            result['valid_number'] = self.valid_check_digits[0]
            if result['valid_expiration_date'] :
                try:
                    self.expiration_date = arrow.get(f"{self.expiration_date}", "YYMMDD").datetime.date()
                except:
                    self.expiration_date = None
            result['expiration_date'] = self.expiration_date             
            if self.mrz_type != "DLFR":
                ...
                if self.mrz_type not in ['MRVA', 'MRVB', 'FR1']:
                    result['check_composite'] = self.check_composite
                if self.mrz_type == 'TD3':
                    result['check_personal_number'] = self.check_personal_number
                result['valid_date_of_birth'] = self.valid_check_digits[2]
                if result['valid_date_of_birth']:
                    self.date_of_birth = arrow.get(f"{self.date_of_birth}", "YYMMDD").datetime.date()
                result['date_of_birth'] = self.date_of_birth                
                if self.mrz_type not in ['FR1', 'MRVA', 'MRVB']:
    ...
    def _parse_td1(self, a, b, c):
        a = self.complete_line(a, 30)
        b = self.complete_line(b, 30)
        c = self.complete_line(c, 30)              
        ...
        try:
            if len(surname_names) < 2:
                ...
                self.surname = surname_names[0].replace('<', ' ').strip()
                self.names = surname_names[1].split("<")[0].replace('<', ' ').strip()
        except:
            self.names = None
            self.surname = None
    ...
    def complete_line(self, line, num):
        letters_needed = num - len(line)
        if letters_needed > 0:
            index = -1
            ...
            if index == -1:
                index = len(line)
            return line[:index]+"<"*letters_needed+line[index:]
        return line
        
        
        

    def _parse_fr1(self, a, b):
        a = self.complete_line(a, 36)
        b = self.complete_line(b, 36)
        len_a, len_b = len(a), len(b)
        self.type = a[0:2]
        self.country = a[2:5]
        self.surname = a[5:30].replace('<', ' ').strip()
        self.number = b[0:12]
        self.creation_year = b[0:2]
        self.creation_month = b[2:4]
        # self.expiration_date = f"{self.creation_year}{self.creation_month}01"
        try:
            self.expiration_date = arrow.get(f"{self.creation_year}{self.creation_month}01", "YYMMDD").shift(years=10).datetime.strftime("%y%m%d")
        except:
            self.expiration_date = None
        self.personal_number = b[4:7]
        self.nationality = b[7:12]
        self.check_expiration_date = b[12]
        self.names = b[13:27].split("<<")[0]
        self.date_of_birth = b[27:33]
        self.check_date_of_birth = b[33]
        self.sex = b[34]
        self.check_number = b[35]
        self.valid_check_digits = [MRZCheckDigit.compute(a + b[:-1]) == self.check_number,
                                    MRZCheckDigit.compute(b[0:12]) == self.check_expiration_date,
                                   MRZCheckDigit.compute(b[27:33]) == self.check_date_of_birth,
                                   ]
        self.valid_line_lengths = [len_a == 36, len_b == 36]
        self.valid_number , self.valid_expiration_date, self.valid_date_of_birth = self.valid_check_digits
        self.valid_score = int(self.valid_number)*100
        return self.valid_score == 100

    def _parse_dlfr(self, a):
        a = self.complete_line(a, 30)
        len_a = len(a)
        a = 'D1FRA'+a[5:]
        self.type = a[0:2]
        self.country = a[2:5]
        self.number = a[5:14]
        self.expiration_date = a[15:21]
        self.surname = a[21:29].replace('<', ' ').rstrip()
        self.check_number = a[29]
        self.valid_check_digits = MRZCheckDigit.compute(a[0:29]) == self.check_number
        self.valid_check_digits = [self.valid_check_digits, self.valid_check_digits]
        self.valid_line_lengths = [len_a == 30]
        self.valid_expiration_date, self.valid_number = self.valid_check_digits
        self.valid_score = int(self.valid_number)*100
        return self.valid_score == 100        
    ...
    def _parse_td3(self, a, b):
        a = self.complete_line(a, 44)
        b = self.complete_line(b, 44)
        len_a, len_b = len(a), len(b)
        self.type = a[0:2]
        self.country = a[2:5]
    ...
    def _parse_mrv(self, a, b, length=44):
        a = self.complete_line(a, length)
        b = self.complete_line(b, length)        
        len_a, len_b = len(a), len(b)
        self.type = a[0:2]
        self.country = a[2:5]
...
class MRZOCRCleaner(object):
    ...
    def __init__(self):
        ...
        FR1 = ['A'*30 + '*'*6,
                "n"*4 + "*"*3 + "n"*6 + "A"*14 + "n"*7 + "a"*1 + "n"*1]
        DLFR = ['*'*14+'n'*7+'A'*8+'n'*1]
        self.FORMAT = {'TD1': TD1, 'TD2': TD2, 'TD3': TD3, 'MRVA': MRV, 'MRVB': MRV, 'FR1': FR1, 'DLFR': DLFR}
